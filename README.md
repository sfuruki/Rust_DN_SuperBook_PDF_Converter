...
- 正式サポート: Windows 11 + WSL2 (Ubuntu) + Docker Desktop + Docker Compose
- 実行形態: Web UI / CLI ともに Docker コンテナ実行を前提
- GPU 利用: NVIDIA GPU は推奨
- 注記: CPU only (`docker-compose.cpu.yml`) と ROCm (`docker-compose.rocm.yml`) は参考構成であり、stable リリースでは未テスト
- 非対象: ネイティブ Windows 実行、Linux/macOS ネイティブ実行は検証対象外

### stable リリース前の推奨チェック

- ローカル: `cd superbook-pdf && task release-preflight`
- GitHub Actions: `Release Dry Run` を `tag_name=vX.Y.Z` で実行
- Branch Protection の必須チェック例:
  `check`, `docker-smoke`, `pr-label-gate`, `workflow-lint`, `dependency-policy`

### 最終リリース直前ステータス

- 実装・設定: 完了（CI / release / dry-run / label / security / dependency policy）
- フォーマット: 完了（`cargo fmt` 適用済み）
- 注意: Windows のローカル PowerShell 環境では `gcc.exe` / `dlltool.exe` 不足により `clippy` / `test` が失敗する場合があるため、最終判定は WSL または GitHub Actions 結果を基準にする
- 最終リリースで未実施: `Release Dry Run` の実行確認、`vX.Y.Z` タグでの本番 `Release` 実行
...
| `--advanced` | off | 高品質処理を有効化 (内部解像度正規化 + カラー補正) |
| `--ocr` | off | 日本語OCR を有効化 |
| `--dpi <N>` | 300 | 出力 DPI |
...
入力PDF
  ...
  ├─ Step 3: AI超解像 (RealESRGAN 2x)
  ├─ Step 4: 180度回転検出・補正
  ├─ Step 5: 傾き補正 (大津二値化 + Hough変換)
  ├─ Step 6: カラー補正 (HSV裏写り抑制)
  ├─ Step 7: グループクロップ (均一マージン)
  ├─ Step 8: PDF生成 (JPEG DCT圧縮)
  └─ Step 9: OCR (YomiToku, --ocr)
  │
  出力PDF
```
